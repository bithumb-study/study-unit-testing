...
- 하나의 동작을 설명하기 위해 테스트 하나로는 충분치 않음. 동작이 복잡할 수록 테스트가 증가 → 이를 극복하기 위해 parameterized test를 통해 테스트를 묶을 수 있음
- 하지만 parameterized test로 묶는다면 테스트 코드는 확실히 줄일 수 있지만 무슨 동작을 테스트해야하는지 모호해진다(가독성과 코드의 양은 반비례 관계)
- 하나의 동작을 설명하기 위해 있는 모든 테스트를 묶기보단 정상동작과 비정상동작을 기준으로 나누는 등 타협점을 찾아야한다.(특히 너무 복잡한 동작이라면 매개화된 테스트를 이용하지 않는다)

## 3.6 검증문 라이브러리를 사용한 테스트 가독성 향상

- 각 언어에 맞춰 검증문 라이브러리를 사용하면 가독성을 향상시킬 수 있다